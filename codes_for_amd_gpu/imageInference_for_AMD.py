...
model.load_state_dict(torch.load('classification_model.pth', map_location=device))  # Cargar el modelo en DirectML
model.to(device)
model.eval()

print("Model loaded successfully on DirectML.")
...
def draw_label(image, label):
    ...
    draw.text((50, 50), label, fill="red")
    print(f'El alimento es {label}')
    return image
